...
describe('CacheItem', () => {
  it('should not be valid after it was forced to be invalid', () => {
    const asyncResultArrived = new ResultArrived(someId, someResult, now)
  ...
  it('should contain a result (and not be invalid) when a result arrived with the right id', () => {
    const asyncResultAwaiting = new AwaitingFirstResult(someId)
    ...
    expect(cacheItemWithResult.asyncResult).toEqual(new ResultArrived(someId, someResult, now))
    expect(cacheItemWithResult.isValid(now)).toEqual(true)
  })

  it('should not contain a result when a result arrived with the wrong id', () => {
    const asyncResultAwaiting = new AwaitingFirstResult(someId)
    const cacheItem = new CacheItem(someInput, asyncResultAwaiting, smallLifetime, true)
