import { GenericAction, isAwaitingResultAction, isResultArrivedAction, AWAITING_RESULT } from '../Action'
...
export const matchAwaitingResult = <A>(): IMatcher<AsyncResult<A>> => (asyncResult: AsyncResult<A>) => {
  if (asyncResult.type === AWAITING_RESULT) {
    return true
  } else {
    return `Input ${JSON.stringify} not of type AWAITING_RESULT`
  }
}

export const matchAwaitingResultAction = (key: string): IMatcher<GenericAction> => (action: GenericAction) => {
...
export const matchResultArrivedAction = (key: string, result: Result): IMatcher<GenericAction> => (action: GenericAction) => {
